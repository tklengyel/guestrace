...
step_cb (vmi_instance_t vmi, vmi_event_t *event) 
{
	...
	struct required_event_data *event_data = (struct required_event_data *)event->data;
	...
	status = vmi_write_8_pa(vmi, event_data->phys_system_call_entry_addr, &event_data->breakpoint_inst);
	if (VMI_FAILURE == status) {
		fprintf(stderr, "Failed to write the break point to syscall at 0x%"PRIx64" in step_cb!\n", event_data->phys_system_call_entry_addr);
		interrupted = 1; 		
		return VMI_EVENT_RESPONSE_NONE;	
	}
	
	status = vmi_write_8_pa(vmi, event_data->phys_sysret_addr, &event_data->breakpoint_inst);
	if (VMI_FAILURE == status) {
		fprintf(stderr, "Failed to write the break point to ret_from_sys_call at 0x%"PRIx64" in step_cb!\n", event_data->phys_sysret_addr);
		interrupted = 1;
		return VMI_EVENT_RESPONSE_NONE;
	}
...
int3_cb (vmi_instance_t vmi, vmi_event_t *event) 
{
	...
	if (rip == event_data->virt_system_call_entry_addr) {
		
		status = vmi_write_8_pa(vmi, event_data->phys_system_call_entry_addr, &event_data->orig_syscall_inst);		/* restore the original instruction for system_call */
		if (VMI_FAILURE == status) {						
	...
	else if (rip == event_data->virt_sysret_addr) {
		
		status = vmi_write_8_pa(vmi, event_data->phys_sysret_addr, &event_data->orig_sysret_inst);	/* restore the original instruction for ret_from_sys_call */
		if (VMI_FAILURE == status) {					
...
status_t
set_up_int3_event (vmi_instance_t vmi, vmi_event_t int3_event, struct required_event_data *event_data)
{
	memset(&int3_event, 0, sizeof(vmi_event_t));			
	SETUP_INTERRUPT_EVENT(&int3_event, 0, int3_cb);	
	int3_event.data = event_data;

	return vmi_register_event(vmi, &int3_event);
}

status_t
set_up_single_step_event (vmi_instance_t vmi, vmi_event_t step_event, struct required_event_data *event_data)
{
	memset(&step_event, 0, sizeof(vmi_event_t));								
	SETUP_SINGLESTEP_EVENT(&step_event, 1, step_cb, 0);		
	step_event.data = event_data;

	return vmi_register_event(vmi, &step_event);
}

status_t
set_up_system_call_entry_int3 (vmi_instance_t vmi, struct required_event_data *event_data)
{
	/*
	...
	status = vmi_get_vcpureg(vmi, &event_data->virt_system_call_entry_addr, MSR_LSTAR, 0);	/* get and store the virtual address for the system_call function which is stored in MSR_LSTAR */
	if (VMI_FAILURE == status) {	
		fprintf(stderr, "Failed to get the system_call() function entry address from MSR_LSTAR!\n");
		goto done;
	}

	event_data->phys_system_call_entry_addr = vmi_translate_kv2p(vmi, event_data->virt_system_call_entry_addr);	/* get and store the physical address of the system_call function form the virtual address*/	
	if (0 == event_data->phys_system_call_entry_addr) {
		fprintf(stderr, "Failed to get the physical address of the system_call() function\n");
		status = VMI_FAILURE;
		goto done;
	}

	status = vmi_read_8_pa(vmi, event_data->phys_system_call_entry_addr, &event_data->orig_syscall_inst);	/* get and store the original first 8 bits of the system_call function */
	if (VMI_FAILURE == status) {		
		fprintf(stderr, "Failed to read original instruction from 0x%"PRIx64"!\n", event_data->phys_system_call_entry_addr);
		goto done;
	}

	status = vmi_write_8_pa(vmi, event_data->phys_system_call_entry_addr, &event_data->breakpoint_inst);	/* write the break point instruction to the first byte of the system_call function */
	if (VMI_FAILURE == status) {				
...
status_t
set_up_sysret_entry_int3 (vmi_instance_t vmi, struct required_event_data *event_data)
{
	/*
 	 *  Gets all necessary addresses and writes the break point instruction
	...
	event_data->virt_sysret_addr = vmi_translate_ksym2v(vmi, "ret_from_sys_call");	/* get and store the virtual address of the ret_from_sys_call function */
	if (0 == event_data->virt_sysret_addr) {
		fprintf(stderr, "Failed to get the virtual address of ret_from_sys_call\n");
		status = VMI_FAILURE;
		goto done;
	}

	event_data->phys_sysret_addr = vmi_translate_kv2p(vmi, event_data->virt_sysret_addr);		/* get and store the physical address of ret_from_syscall */
	if (0 == event_data->phys_sysret_addr) {
		fprintf(stderr, "Failed to get the physical address of ret_from_sys_call\n");
		status = VMI_FAILURE;
		goto done;
	}

	status = vmi_read_8_pa(vmi, event_data->phys_sysret_addr, &event_data->orig_sysret_inst);	/* get and store the first byte of ret_from_sys_call */
	if (VMI_FAILURE == status) {   	
		fprintf(stderr, "Failed to read original instruction from 0x%"PRIx64"!\n", event_data->phys_sysret_addr);
		goto done;
	}
 ...
 */
void
restore_original_instructions (vmi_instance_t vmi, struct required_event_data *event_data)
{
	/*
	...
	status = vmi_write_8_pa(vmi, event_data->phys_system_call_entry_addr, &event_data->orig_syscall_inst);	/* restore the original byte of system_call */
	if (VMI_FAILURE == status) {
		fprintf(stderr, "Failed to write original syscall instruction back to memory, your VM may need to be restarted!\n");
	}
	
	status = vmi_write_8_pa(vmi, event_data->phys_sysret_addr, &event_data->orig_sysret_inst);	/* restore the original byte of ret_from_sys_call */
	if (VMI_FAILURE == status) {
...
main (int argc, char *argv[]) 
{
	...
	struct required_event_data event_data;
	event_data.breakpoint_inst = 0xCC;
	...
	status = set_up_system_call_entry_int3(vmi, &event_data);
	if (VMI_FAILURE == status) {
		goto done;
	}

	status = set_up_sysret_entry_int3(vmi, &event_data);
	if (VMI_FAILURE == status) {
		goto done;
	}
	status = set_up_int3_event(vmi, int3_event, &event_data);
	if (VMI_FAILURE == status) {	
		fprintf(stderr, "Failed to setup the int3 event!");
		goto done;
	}	

	status = set_up_single_step_event(vmi, step_event, &event_data);
	if (VMI_FAILURE == status) {
	...
	if (NULL != vmi) {
		restore_original_instructions(vmi, &event_data);
